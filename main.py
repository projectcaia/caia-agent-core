...
async def debug():
    ...
    return {
        ...
        "endpoints": [
            ...
            "/agent/orchestrate", "/agent/report",
            "/agent/proxy/n8n/{path}"
        ],
    }
...
@app.post("/agent/report", operation_id="postAgentReport")
async def post_agent_report(request: Request, authorization: Optional[str] = Header(default=None)):
    check_bearer(authorization, CAIA_AGENT_KEY)
    body = await request.json()
    print(f"📨 [REPORT] From n8n → {body}")
    return {"status": "received", "timestamp": now_iso()}
