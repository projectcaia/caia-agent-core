...
class N8NClient:
    ...
    Supports both API key and BasicAuth authentication based on
    provided credentials. When BasicAuth credentials are supplied,
    they will be preferred over the API key. When only an API key
    is provided, the client will use the `X-N8N-API-KEY` header as
    recommended by n8n documentation. The "Authorization: Bearer"
    header has been removed to avoid conflicting with BasicAuth on
    n8n instances that do not support Bearer tokens.
    """
    ...
    def _headers(self) -> Dict[str, str]:
        ...
        # Prefer BasicAuth if credentials are provided
        ...
        # Neither authentication mechanism is configured
        raise ValueError(
            "No authentication configured: provide either N8N_API_KEY or N8N_BASIC_AUTH_USER/PASSWORD"
        )
